...
`tc_test_orchestrate.py` covers the decision table and `tc_check_examples.py` covers
...
def container(title: str, payload: dict) -> str:
    return (f"# {title}\n\nGenerated by tc_smoke_pipeline.py — safe to delete.\n\n"
...
def setup(repo: Path, target: str) -> tuple[Path, str]:
    ...
    (plans / "plan-v1.md").write_text(container("Test Plan: " + target, {
        ...
        "status": "READY",
        "context": {"target_sha": sha, "notes": ["written by tc_smoke_pipeline.py"]},
        "scenarios": [{
            "id": "TC01",
            "change": "NEW",
...
def main() -> int:
    ...
    failures = []
    steps = [("tc_run_tests.py", ["--repeat", "1"], "tests-r1.md"),
             ("tc_coverage.py", [], "coverage-r1.md")]
    if not args.skip_mutation:
        steps.append(("tc_mutation.py", [], "mutation-r1.md"))
