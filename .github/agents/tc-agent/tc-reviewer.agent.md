...
- every method the Generator wrote or repaired carries the §A1 Javadoc:
  `@aiGenerated`; `@mode` equal to `run.json` mode; `@interactive` exactly when
  `run.json` says so; in legacy `@characterizes <TargetClass>@<sha>` where the
  sha matches the plan's `context.target_sha`; no `@characterizes` in
  spec-driven; `notes` of the scenario present as `@note` lines. Any deviation
  is a `metadata_defect` finding against the implementation, naming the method
  and the tag. The run's `derive-state.md` lists defects the parser already
  found in older tests — do not blame this generation for those;
- no tags on the test class, no `// TC-nn` or scenario ids in the code;
