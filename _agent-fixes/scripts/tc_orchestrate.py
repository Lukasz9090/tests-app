...
def open_feedback_review(directory: Path, up_to_version: int):
    """The newest review at or below `up_to_version`, and its still-open
    implementation feedback.

    A review's own `feedback.implementation` is the authoritative open set for
    its lineage — a later review supersedes an earlier one — so only the NEWEST
    readable review decides. This keeps a real, unaddressed finding from being
    dropped on a plan bump or a COMPLETE plan, WITHOUT re-opening a finding that
    a later review already cleared (green again, no feedback).
    """
    for version in range(up_to_version, 0, -1):
        for iteration in reversed(_review_iterations(directory, version)):
            ...
            if "__error__" in review:
                continue               # unreadable: fall back to the next-newest
            # An ACCEPT, or a review that carries no implementation feedback
            # (a REPAIR_PLAN once the tests are green again), closes the
            # implementation concerns even when an EARLIER review raised them.
            if review.get("decision") in DECISION_ACCEPT:
                return None, None
            return (path, review) if _impl_feedback(review) else (None, None)
    return None, None
