...
- Between runs, `tc_orchestrate.py start` derives what a target needs from the
  ...
  `tc-agent` metadata lines in their Javadoc, git, coverage and mutations.
...
METHOD (never on the class), as plain `tc-agent-<key>: <value>` lines — not
Javadoc `@tags`, so no IDE flags them. The full rules are in
`tc-test-conventions.md` §A; the lines are:
