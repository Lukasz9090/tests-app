...
def test_complete_plan_feedback_closed_by_later_review():
    @case("COMPLETE plan whose earlier finding was fixed and re-reviewed clean -> DONE")
    def _(repo):
        repo.plan(1, "READY", [PENDING])
        repo.report(1, 1)
        repo.review(1, 1, "REPAIR_IMPLEMENTATION", feedback=FEEDBACK)
        repo.report(1, 2)
        repo.review(1, 2, "REPAIR_PLAN")           # green again, no impl feedback
        repo.plan(2, "COMPLETE", [COVERED], based_on=1)
        state = repo.state()
        expect("next_action", state["next_action"], "DONE")
        expect("reason says implemented", "already implemented" in state["reason"], True)
