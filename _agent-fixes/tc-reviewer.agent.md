...
- a `SKIPPED` or `OBSOLETE` result follows the plan flags for that id, not the
  Generator's convenience. `SKIPPED` is ALSO legitimate when this is a repair
  iteration and an earlier generation report of the SAME plan version recorded
  that id as `IMPLEMENTED`, its `// TC-nn` method still exists and passed in
  step 1 — that is "already implemented in an earlier iteration, unchanged now",
  not a missing test. Require the `notes` to name that earlier report;
...
4. `failure_phase: assertion` in mode `legacy` → **REPAIR_IMPLEMENTATION**,
   ...
   **REPAIR_PLAN**.
5. `failure_phase: assertion` in mode `spec-driven` →
